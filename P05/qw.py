...
def compare_bigrams(target_word, unique_words_list):
    ...
    for word in unique_words_list:
        ...
        if common_bigrams:
            candidate_list.append((word, len(common_bigrams), word_bigrams_list))  # Append word, count of matching bigrams, and bigrams
...
def main():
    ...
    # Display the candidate list and their bigrams
    print("\nCandidate List (word, matching bigrams count, bigrams):")
    for word, match_count, bigrams_list in candidate_list:
        print(f"{word}: {match_count} matching bigrams")
        print(f"Bigrams for '{word}': {bigrams_list}\n")
    
    # Write candidate list with bigrams to file
    with open('Candidate_List.txt', 'w') as file:
        for word, match_count, bigrams_list in candidate_list:
            file.write(f"{word}: {match_count}, Bigrams: {bigrams_list}\n")
